...
当前第一轮目录迁移已经完成，仓库内部实现已统一收口到 `app/`。本文档描述的是现在生效的结构规范和后续新增能力的推荐落点。
...
- `scripts/`（面向运维与 agent 的外部执行边界）
- `deploy/`（部署与运行配置）

原因：

- 对外入口仍然稳定有效
- 这些目录不属于当前后端分层收口的重复实现
- 当前最重要的是在 `app/` 下继续累积新能力
...
当前仓库内部实现已经统一收口到 `app/`。后续新增能力不再创建旧路径兼容层；若未来需要对外提供兼容接口，应以明确版本策略管理，而不是在仓库内部长期保留重复实现。
...
2. 不把数据库访问直接写进路由层
3. 不把博主视频同步逻辑直接塞进现有抓取服务模块，应建立独立 service
